...
namespace CodeCheckingConcreteExample.Concrete
{
   ...
   public class ConcreteSectionDesign
   {
      ...
      public void DataVeryfication()
      {
         ...
         if (!(Autodesk.Revit.DB.BuiltInCategory.OST_ColumnAnalytical == elementType || Autodesk.Revit.DB.BuiltInCategory.OST_BeamAnalytical == elementType))
         {
            throw new Exception("Types: Element is not supported by this code.");
         }
         ...
         if (Math.Min(longReinforcementFy, transReinforcementFy) < 1e4)
         {
            throw new Exception(Properties.Resources.ResourceManager.GetString("ErrReinforcementYieldStress"));
         }
         if (Math.Min(transReinforcementArea, longReinforcementArea) < 1e-7)
         {
      ...
      /// <summary>
      /// Reduced the number of the internal forces if possible for the calculation of longitudinal reinforcement.</br> 
      ///  Separates the list of SLS and ULS. </br>
      /// The new lists of the internal forces are stored in internalForcesULSLrb, internalForcesSLSLrb
      /// </summary>
            ...
            case ConcreteTypes.CalculationType.BendingY:
               {
                  ...
                  if (forcesMinMUls.MomentMy < -Double.Epsilon || longReinforcementInternalForcesULS.Count() == 0)
                  ...
                  if (forcesMinMSls.MomentMy < -Double.Epsilon || longReinforcementInternalForcesSLS.Count() == 0)
            ...
            case ConcreteTypes.CalculationType.EccentricBendingY:
               {
                  ...
                  foreach (InternalForcesContainer forces in internalForces)
                  {
                     if (ForceLimitState.Sls == forces.LimitState)
                     {
                        vNMSls.Add(new Point2D(forces.ForceFx, forces.MomentMy));
                     }
                     else
                     {
                        vNMUls.Add(new Point2D(forces.ForceFx, forces.MomentMy));
                     }
                  }
                  ...
                  {
                     longReinforcementInternalForcesULS.Add(internalForces[index]);
                     if (!symmetricalReinforcementPreferable)
                     {
                        absForceFx = Math.Abs(internalForces[index].ForceFx);
                        if (absForceFx > Double.Epsilon)
                        {
                           eccentricity = Math.Abs(internalForces[index].MomentMy) / absForceFx;
                  ...
                  {
                     longReinforcementInternalForcesSLS.Add(internalForces[index]);
                     if (!symmetricalReinforcementPreferable)
                     {
                        absForceFx = Math.Abs(internalForces[index].ForceFx);
                        if (absForceFx > Double.Epsilon)
            ...
            case ConcreteTypes.CalculationType.AxialForce:
               {
                  ...
                  if (forcesMinNUls.ForceFx < -Double.Epsilon || longReinforcementInternalForcesULS.Count() == 0)
                  ...
                  if (forcesMinNSls.ForceFx < -Double.Epsilon || longReinforcementInternalForcesSLS.Count() == 0)
      ...
      /// <summary>
      /// Reduced the number of the internal forces if possible for the calculation of transversal reinforcement.</br>
      /// Separates the list of SLS and ULS.</br>
      /// The new lists of the internal forces are stored in internalForcesULSTrb
      /// </summary>
      public void ReduceInternalForcesForTransversalReinforcement()
      {
         ...
         if (ConcreteTypes.CalculationType.ShearingZ == transReinforcementCalculationType)
         {
            ...
            if (forcesMinVUls.ForceFz < -Double.Epsilon || transReinforcementInternalForcesULS.Count() == 0)
      ...
      {
         if (elementType == Autodesk.Revit.DB.BuiltInCategory.OST_BeamAnalytical) // minimum reinforcement for beam
         {
            double minimumReinforcement = 0.001 * sectionGeometry.Area;        // 0.01% on each side with reinforcement
         ...
         else if (elementType == Autodesk.Revit.DB.BuiltInCategory.OST_ColumnAnalytical) // minimum reinforcement for column
         {
            ...
            double minimumReinforcement = 0.005 * sectionGeometry.Area; // 0.1% as minimum area
      ...
      private void CalculateStiffness()
      {
         ...
         {
            minStiffness = sectionGeometry.MomentOfInertiaY * concreteParameters.ModulusOfElasticity;
            SetMaterialParameters(ForceLimitState.Sls); // sets material properties for serviceability limit state
            if (longReinforcementInternalForcesSLS.Count() > 0)
            {
               noSLS = false;
      ...
      private double CalculateStiffnesSLS(InternalForcesContainer forces, double creepCofficient)
      {
         ...
         double momentOfInertiaConcreteSection = sectionGeometry.MomentOfInertiaY;
         double stiffnes = momentOfInertiaConcreteSection;
         if (!IsZeroForces(forces, true))
         {
            double concreteTensionLimit = 0.3 * Math.Pow(concreteParameters.DesignStrength * 1e-6, 2.0 / 3.0) * 1e6;
            double forcesToCrackingForces = verificationHelper.ForcesToCrackingForces(forces, concreteTensionLimit);
            if (forcesToCrackingForces < 1.0)
            {
               double momentOfInertiaCrackingConcreteSection = verificationHelper.InertiaOfCrackingSection(forces);
               stiffnes = forcesToCrackingForces * forcesToCrackingForces * momentOfInertiaConcreteSection +
                        (1 - forcesToCrackingForces * forcesToCrackingForces) * momentOfInertiaCrackingConcreteSection;
            }
         }
      ...
      private void CalculateLongitudinalReinforcementSimplify(ref InternalForcesContainer forces)
      {
         ...
         if (maxOneSideReforcementMoment > absM)
         {
            ...
            if (!CalculationUtility.RootsOfQuadraticEquation(a, b, c, ref xM1, ref xM2))
               throw new Exception("Invalid CalculateLongitudinalReinforcementSimplify delta <= 0.0.");
            double coeff = xM1 * xM2;
            double xM = coeff >= Double.Epsilon ? Math.Min(xM1, xM2) : Math.Max(xM1, xM2);
            if (xM <= 0.0)
               throw new Exception("Invalid CalculateLongitudinalReinforcementSimplify xM <= 0.0.");
      ...
      private void FindOptimalLongitudinalReinforcementUniaxialSymmetrical(ref double safetyFactor, ref InternalForcesContainer forces)
      {
         ...
         {
            reinforcementIncrase = 10.0 * CalculationUtility.MinimumReinforcement();
            ...
            if (resultNotOK)
            {
               ...
               {
                  reinforcementIncrase = (2.0 * CalculationUtility.MinimumReinforcement()) / increasesafetyFactorTopBottom;
               }
               else
               {
         ...
         if (resultNotOK)
         {
            throw new Exception("Too many iteration. FindOptimalLongitudinalReinforcementUniaxialSymmetrical.");
         }
         else
         {
      ...
      private void FindOptimalLongitudinalReinforcementUniaxialUnsymmetrical(ref double safetyFactor, ref InternalForcesContainer forces)
      {
         ...
         while (!CalculationUtility.IsIterEnd(++i) && resultNotOK)
         {
            reinforcementIncraseBottom = reinforcementIncraseTop = 10.0 * CalculationUtility.MinimumReinforcement();
         ...
         if (resultNotOK)
         {
            throw new Exception("Too many iteration. FindOptimalLongitudinalReinforcementUniaxialUnsymmetrical.");
         }
         else
         {
            resultNotOK = !CalculationUtility.IsSafetyOptimal(safetyFactor);
      ...
      public void AdjustReinforcementIncrase(ref double reinforcementIncrase)
      {
         ...
         {
            reinforcementIncrase = Math.Max(reinforcementIncrase, 10.0 * CalculationUtility.MinimumReinforcement());
      ...
      {
         double minimumIncreaseReinforcement = CalculationUtility.MinimumReinforcement();
         if (verificationHelper == null)
         {
            verificationHelper = RcVerificationHelperUtility.CreateRcVerificationHelperUtility(sectionType, ref sectionGeometry, longReinforcementTopCover, longReinforcementBottomCover);
            SetMaterialParameters(forces.LimitState);
      ...
      {
         // Checking capacity for bigger reinforcement on the top and on the bottom
         bool onlyTopBottom = false;
         bool onlyRightLeft = false;
         bool resultNotOK = true;
         double curentReinforcement = longitudinalReinforcement.CurrentAsTop + longitudinalReinforcement.CurrentAsBottom + longitudinalReinforcement.CurrentAsRight+ longitudinalReinforcement.CurrentAsLeft;
         ...
         verificationHelper.SetReinforcement(longitudinalReinforcement.CurrentAsTop + reinforcementIncraseTopBottom, longitudinalReinforcement.CurrentAsBottom + reinforcementIncraseTopBottom, longitudinalReinforcement.CurrentAsRight, longitudinalReinforcement.CurrentAsLeft);
         double safetyFactorTopBottom = onlyRightLeft ? 0.0 : verificationHelper.SafetyFactor(forces);
         verificationHelper.SetReinforcement(longitudinalReinforcement.CurrentAsTop, longitudinalReinforcement.CurrentAsBottom, longitudinalReinforcement.CurrentAsRight + reinforcementIncraseRightLeft, longitudinalReinforcement.CurrentAsLeft + reinforcementIncraseRightLeft);
         ...
         double increaseSafetyFactorTopBottom = 1.0;
         double increaseSafetyFactorRightLeft = 1.0;
         while (!CalculationUtility.IsIterEnd(++i) && resultNotOK)
         {
            increaseSafetyFactorTopBottom = safetyFactorTopBottom - safetyFactor;
            ...
            if (resultNotOK)
            {
               // check capacity separately for top and bottom or left and right
               reinforcementIncraseTopBottom = CalculationUtility.MinimumReinforcement();
               verificationHelper.SetReinforcement(asTopBottom[1] + reinforcementIncraseTopBottom, asTopBottom[1] + reinforcementIncraseTopBottom, asRightLeft[1], asRightLeft[1]);
               safetyFactorTopBottom = onlyRightLeft ? 0.0 : verificationHelper.SafetyFactor(forces);
               resultNotOK = !CalculationUtility.IsSafety(safetyFactorTopBottom);
               if (resultNotOK)
               {
                  reinforcementIncraseRightLeft = CalculationUtility.MinimumReinforcement();
                  verificationHelper.SetReinforcement(asTopBottom[1], asTopBottom[1], asRightLeft[1] + reinforcementIncraseRightLeft, asRightLeft[1] + reinforcementIncraseRightLeft);
            ...
            {
               // reinforcement is to high  - copied from [1] to [2]
               asTopBottom[2] = asTopBottom[1];
               asRightLeft[2] = asRightLeft[1];
            }
         ...
         if (resultNotOK)
         {
            throw new Exception("Too many iteration. CalculateLongitudinalReinforcementBiaxial.");
         }
         else
         {
            resultNotOK = !CalculationUtility.IsSafetyOptimal(safetyFactor);
      ...
      private void BisectionForReinforcementAdjustment(ref InternalForcesContainer forces, double[] asTop, double[] asBottom, double[] asRight = null, double[] asLeft = null)
      {
         ...
         if ((asTop.Length != 3 || asTop.Length != 3) || (!onlyTopBottomReinforcemet && (asRight.Length != 3 || asLeft.Length != 3)))
            throw new Exception("Invalid parameter in BisectionForReinforcementAdjustment.");
         bool resultNotOK = true;
      ...
      private void CalculateTransversalReinforcementPureTorsion(double T)
      {
         ...
         {
            throw new Exception("Invalid section for torsion. Only R cross-sections can be used on this path.");
         }
      }
      /// <summary>
