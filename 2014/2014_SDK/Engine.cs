...
namespace CodeCheckingConcreteExample.Engine
{
   ...
   public class Engine
   {
      ...
      protected void CalculateSetOfElements(List<Tuple<ElementId, ResultStatus>> listElementStatus, Server.Server server, Autodesk.Revit.DB.CodeChecking.ServiceData data)
      {
         ...
         if (NotificationService.ProgressBreakInvoked() == false)
            CalculateElementList(listElementFiltered, parameters, maxNumberOfThreads);
      ...
      protected void CalculateElementList(List<ObjectDataBase> listElementData, CommonParametersBase parameters, int maxNumberOfThreads)
      {
         ICalculationScenario scenario = EngineData.CreateCalculationScenario();
         List<ICalculationObject> scenarioList = scenario.CalculationScenarioList();
         ...
         foreach (ICalculationObject calcObj in scenarioList)
         {
            ...
            {
               if (calcObj.Categories.Contains(elemData.Category))
               {
         ...
         {
            foreach (ObjectDataBase element in listElementData)
            {
         ...
         else
         {
            ...
            Parallel.ForEach(listElementData, new ParallelOptions() { MaxDegreeOfParallelism = maxNumberOfThreads }, (element, elLoopState) =>
               {
                  bool errorOccured = false;
                  foreach (ICalculationObject calcObj in scenarioList)
                  {
