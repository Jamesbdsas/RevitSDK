   ...
   public class FabricationPartLayout : IExternalCommand
   {
          ...
          , ref string message, ElementSet elements)
      {
         try
         {
            ...
            FabricationService service = m_services[0];  

            FabricationServiceButton bt_transition = locateButton(service, 0, "Transition");
            FabricationServiceButton bt_sqBend = locateButton(service, 0, "Square Bend");
            FabricationServiceButton bt_tap = locateButton(service, 0, "Tap");
            FabricationServiceButton bt_rectStraight = locateButton(service, 0, "Straight");
            FabricationServiceButton bt_radBend = locateButton(service, 0, "Radius Bend");
            FabricationServiceButton bt_flatShoe = locateButton(service, 1, "Flat Shoe");
            FabricationServiceButton bt_tube = locateButton(service, 1, "Tube");
            FabricationServiceButton bt_90bend = locateButton(service, 1, "Bend - 90");
            FabricationServiceButton bt_45bend = locateButton(service, 1, "Bend - 45");
            FabricationServiceButton bt_rectTee = locateButton(service, 0, "Tee");
            FabricationServiceButton bt_sqToRound = locateButton(service, 0, "Square to Round");
            FabricationServiceButton bt_reducer = locateButton(service, 1, "Reducer - C");
            FabricationServiceButton bt_curvedBoot = locateButton(service, 1, "Curved Boot");
            FabricationServiceButton bt_hangerBearer = locateButton(service, 4, "Rectangular Bearer");
            FabricationServiceButton bt_hangerRound = locateButton(service, 4, "Round Duct Hanger");
            ...
            using (Transaction tr = new Transaction(m_doc, "Create Layout"))
            {
               ...
               // now override the condition by specifying it
               double lower = bt_90bend.GetConditionLowerValue(1); // note this value is higher than 10"
               double higher = bt_90bend.GetConditionUpperValue(1);
               FabricationPart pt_90bend2 = FabricationPart.Create(m_doc, bt_90bend, 1, levelOne.Id);
